...
namespace djb2 {
	...
	constexpr uint32_t Hash(const char* string, const uint32_t hash = Detail::BEGIN_HASH_VALUE) {
		return (string[0] == '\0') ? 0 : ((hash << 5) + hash) + string[1] + Hash(&string[1], hash);
	}
}

// Source: https://gist.github.com/xoorath
...
namespace CRC32 {
	...
	namespace Internal {
		...
		constexpr unsigned Hash(const char *str, unsigned len, unsigned hash = 0xffffffff)
		{
			return len > 0 ? Hash(str + 1, len - 1, table[((hash) ^ ((unsigned char)*str)) & 0xff] ^ ((hash) >> 8)) : hash;
		}
	...
	constexpr unsigned Hash(const char *str, unsigned len)
	{
		return ~Internal::Hash(str, len);
	}

	constexpr unsigned Hash(const char *str)
	{
		return ~Internal::Hash(str, Internal::StrLen(str));
	}
}
